...
def fetch_health_data(client: Garmin) -> dict:
    """
    Fetch the last 7 days of health metrics from Garmin Connect.
    ...
    Returns a structured dict with:
      ...
      - activities: last 10 activities (type, duration, distance, HR, calories)

    Each day's entry stores None for any metric the device didn't record,
    rather than raising an exception — not all devices support all fields.
    """
    today = date.today()
    ...
    # --- Last 10 activities (not day-by-day, just a flat list) ---
    try:
        raw_acts = client.get_activities(0, 10)
        for act in raw_acts:
...
def format_health_summary(health_data: dict) -> str:
    ...
    for day in health_data["daily_stats"]:
        ...
        if day.get("calories_total") is not None:
    ...
    lines.extend(["", "SLEEP (most recent first):"])

    for s in health_data["sleep"]:
        if "error" in s or not s.get("total_seconds"):
            lines.append(f"  {s['date']}: [no data]")
            continue

        parts = [f"  {s['date']}:", _seconds_to_hm(s["total_seconds"]) + " total"]
        if s.get("deep_seconds"):
            parts.append("deep " + _seconds_to_hm(s["deep_seconds"]))
        if s.get("rem_seconds"):
            parts.append("REM " + _seconds_to_hm(s["rem_seconds"]))
        if s.get("light_seconds"):
            parts.append("light " + _seconds_to_hm(s["light_seconds"]))
        if s.get("score") is not None:
            parts.append(f"score {s['score']}/100")
        lines.append(" | ".join(parts))

    lines.extend(["", "RECENT ACTIVITIES:"])

    if not health_data.get("activities"):
